...
function RoleWorkspace({ role, health, localPreview, currentUserId, onChangeRole }: { role: AppRole; health: BackendHealth; localPreview: boolean; currentUserId?: string; onChangeRole: () => void }) {
  const { data, loading, notice, setNotice, updateAttendance, addUpdate, sendMessage, setConsent } = useWorkspaceStore({ cloud: !localPreview, userId: currentUserId });
  ...
  return <main className="platform-shell">
    ...
    <div className="platform-content">
      ...
      {role === 'parent' && selected && <section className="workspace-panel workspace-consents"><div className="workspace-panel__heading"><div><p className="platform-eyebrow">Privacy</p><h2>Permissions</h2></div><ShieldCheck aria-hidden="true" /></div>{Object.entries(data.consents).map(([key, enabled]) => <label key={key}><span><strong>{key === 'photos' ? 'Learning photos' : key === 'localTrips' ? 'Local walking trips' : 'Emergency care'}</strong><small>Update this preference at any time.</small></span><input type="checkbox" checked={enabled} onChange={event => setConsent(selected.id, key, event.target.checked)} /></label>)}</section>}
