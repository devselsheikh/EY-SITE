...
function RoleWorkspace({ role, health, localPreview, onChangeRole }: { role: AppRole; health: BackendHealth; localPreview: boolean; onChangeRole: () => void }) {
  const { data, notice, setNotice, updateAttendance, addUpdate, sendMessage, setConsent } = useWorkspaceStore();
  const visibleChildren = role === 'parent' ? data.children.slice(0, 1) : role === 'teacher' ? data.children.filter(child => child.keyPerson === 'Sarah Al-Masri') : data.children;
  ...
  return <main className="platform-shell">
    <header className="platform-bar"><Link to="/" className="platform-brand" aria-label="Early Years home"><span className="platform-brand__mark">EY</span><span>Early Years</span></Link><div className="platform-bar__actions"><span className={`platform-status platform-status--${health.state}`}>{health.state === 'online' ? <Database aria-hidden="true" /> : <CloudOff aria-hidden="true" />}{health.state === 'online' ? 'Cloud online' : 'Local safe mode'}</span>{localPreview && <button className="platform-button platform-button--quiet" onClick={onChangeRole}>Switch role</button>}</div></header>
